...
// Get my KPIs
router.get('/kpis/my-kpis', authenticateToken, (req, res) => {
  try {
    const kpis = db.prepare(`
...
router.get('/kpis/user/:userId', authenticateToken, (req, res) => {
  try {
    const kpis = db.prepare(`
      SELECT * FROM kpis WHERE user_id = ? ORDER BY created_at DESC
...
// Create KPI
router.post('/kpis', authenticateToken, (req, res) => {
  try {
...
// Update KPI
router.put('/kpis/:id', authenticateToken, (req, res) => {
  try {
...
// Delete KPI
router.delete('/kpis/:id', authenticateToken, (req, res) => {
  try {
...
router.get('/notes/for/:userId', authenticateToken, (req, res) => {
  try {
...
router.get('/notes/my-notes', authenticateToken, (req, res) => {
  try {
    const notes = db.prepare(`
      SELECT pn.*, a.name as author_name
      FROM performance_notes pn
...
router.delete('/notes/:id', authenticateToken, (req, res) => {
  try {
    db.prepare('DELETE FROM performance_notes WHERE id = ? AND author_id = ?').run(req.params.id, req.user.id);
...
// Get all PIPs (admin)
router.get('/pips', authenticateToken, (req, res) => {
  try {
...
// Get active PIPs
router.get('/pips/active', authenticateToken, (req, res) => {
  try {
    const pips = db.prepare(`
      SELECT p.*,
             u.name as user_name, u.department, u.designation,
...
// Get my PIP
router.get('/pips/my-pip', authenticateToken, (req, res) => {
  try {
...
// Get PIP by ID
router.get('/pips/:id', authenticateToken, (req, res) => {
  try {
    const pip = db.prepare(`
      SELECT p.*,
...
// Create PIP
router.post('/pips', authenticateToken, (req, res) => {
  try {
    ...
    const pip = db.prepare('SELECT * FROM pips WHERE id = ?').get(id);
...
// Update PIP
router.put('/pips/:id', authenticateToken, (req, res) => {
  try {
    ...
    const pip = db.prepare('SELECT * FROM pips WHERE id = ?').get(req.params.id);
    res.json(pip);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
// Add PIP checkpoint
router.post('/pips/:id/checkpoints', authenticateToken, (req, res) => {
  try {
...
// Get PIP checkpoints
router.get('/pips/:id/checkpoints', authenticateToken, (req, res) => {
  try {
    const checkpoints = db.prepare(`
      SELECT pc.*, u.name as reviewed_by_name
      FROM pip_checkpoints pc
...
router.get('/recognitions', authenticateToken, (req, res) => {
  try {
...
// Get my recognitions
router.get('/recognitions/my-recognitions', authenticateToken, (req, res) => {
  try {
    const recognitions = db.prepare(`
...
// Create recognition
router.post('/recognitions', authenticateToken, (req, res) => {
  try {
...
// Delete recognition
router.delete('/recognitions/:id', authenticateToken, (req, res) => {
  try {
    db.prepare('DELETE FROM recognitions WHERE id = ? AND nominator_id = ?').run(req.params.id, req.user.id);
...
router.get('/dashboard', authenticateToken, (req, res) => {
  try {
    ...
    stats.activePips = activePips.count;

    // Recent recognitions count
    const recentRecognitions = db.prepare(`
      SELECT COUNT(*) as count FROM recognitions
      WHERE created_at >= datetime('now', '-30 days')
    `).get();

    stats.recentRecognitions = recentRecognitions.count;

    // Goals progress
    const goalsStats = db.prepare(`
      SELECT
        COUNT(*) as total,
